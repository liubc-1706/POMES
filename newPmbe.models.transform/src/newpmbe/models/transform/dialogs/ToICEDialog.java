...
import newpmbe.models.icemodel.transformation.Tranform;
...
public class ToICEDialog extends Dialog {
	private Composite topComp;
	private Text text1;
	private Text text2;
	
	public ToICEDialog(Shell parentShell) {
		super(parentShell);
	}
	...
	protected Control createDialogArea(Composite parent){
		...
		label1.setText("请选择要转换的PMBE过程模型：");
	...
	protected void buttonPressed(int buttonId) {
		...
		if(buttonId==IDialogConstants.OK_ID){
    		...
    		String icePath=text2.getText();
    		if(iproPath.length()>0&&icePath.length()>0){
    			File project=new File(iproPath);
    			File mainProcessFile=new File(iproPath+"/"+project.getName()+"_diagram");
    			File genFile=new File(icePath);
    			if(!genFile.exists()){
    				MessageDialog.openError(this.getShell(), "error","存放路径不存在!");
    				return;
    			}
    			if(!mainProcessFile.exists()){
    				MessageDialog.openInformation(null, null, "test1");
    				MessageDialog.openError(this.getShell(), "error", "PMBE过程模型出错!");
    				return;
    			}
    			else{
    				Tranform trans=new Tranform();
    				trans.proname=project.getName();
    				trans.storePath=icePath+"\\"+project.getName();
    			
    				trans.processFileParse(mainProcessFile,"main page");
    				trans.addtableset();
