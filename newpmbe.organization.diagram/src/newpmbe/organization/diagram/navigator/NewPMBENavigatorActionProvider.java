...
import newpmbe.organization.diagram.part.NewPMBEDiagramEditor;
		...
		public final void selectionChanged(IStructuredSelection selection) {
			...
			if (selection.size() == 1) {
				...
				if (selectedElement instanceof NewPMBENavigatorItem) {
					...
					NewPMBENavigatorItem item = (NewPMBENavigatorItem) NewPMBENavigatorContentProvider
							.getHashMap().get(selectedElement);
					if (item.getView() instanceof DiagramImpl) {
						Diagram diagram = (Diagram) item.getView();
						if (EMODiagramEditPart.MODEL_ID
								.equals(NewPMBEVisualIDRegistry
										.getModelID(diagram))) {
							myDiagram = diagram;
							...
							String projectExtension = project
									.getFileExtension();
							//去掉功程名称后面的扩展名
							String projectName = project.getName().substring(
									0,
									project.getName().length()
											- projectExtension.length() - 1);
							myFile = project.getFile(projectName
									+ ".organization_diagram"); //$NON-NLS-1$
