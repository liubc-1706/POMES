...
public class WizardFileSystemResourceExportPage1 extends
    ...
    //messages
    private static final String SELECT_DESTINATION_MESSAGE = "文件系统";

    private static final String SELECT_DESTINATION_TITLE = "将资源导入本地文件系统中";

    
    private final static String SELECT_TYPES_TITLE = "选择类型...";
    

    private final static String SELECT_ALL_TITLE = "选择所有";

    private final static String DESELECT_ALL_TITLE = "取消所有";
...
//        setButtonLayoutData(deselectButton);
    ...
    protected void createDestinationGroup(Composite parent) {
        ...
        destinationBrowseButton = new Button(destinationSelectionGroup,
                SWT.PUSH);
        destinationBrowseButton.setText("浏览...");
    ...
    protected void createDirectoryStructureOptions(Composite optionsGroup, Font font) {
        ...
        createDirectoryStructureButton = new Button(optionsGroup, SWT.RADIO
                | SWT.LEFT);
        createDirectoryStructureButton.setText("创建文件的目录结构");
    ...
    protected void createOverwriteExisting(Group optionsGroup, Font font) {
        ...
        overwriteExistingFilesCheckbox = new Button(optionsGroup, SWT.CHECK
                | SWT.LEFT);
        overwriteExistingFilesCheckbox.setText("覆盖现有文件时不弹出警告");
    ...
    protected boolean ensureDirectoryExists(File directory) {
        if (!directory.exists()) {
            if (!queryYesNoQuestion("目的文件夹不存在，是否要创建？")) {
				return false;
			}

            if (!directory.mkdirs()) {
                displayErrorDialog("目的文件夹无法被创建");
                giveFocusToDestination();
                return false;
            }
        }
    ...
    protected boolean ensureTargetIsValid(File targetDirectory) {
        if (targetDirectory.exists() && !targetDirectory.isDirectory()) {
            displayErrorDialog("目的文件夹已作为文件存在");
            giveFocusToDestination();
            return false;
        }
    ...
    protected boolean validateDestinationGroup() {
        ...
        if (conflictingContainer == null) {
            ...
            setErrorMessage(NLS.bind("目的文件夹在{0}处发生冲突", conflictingContainer));
            giveFocusToDestination();
            return false;
        }
