...
import newpmbe.product.ProcessManager;
...
public class ProductDialog extends Dialog {
	...
	Button showProducts;//"显示所有产品"按钮
	
	//以下为４个复选框按钮，查询时用于选择产品类型
	Button b1;//document
	Button b2;//artifact
	Button b3;//product package
	Button b4;//other
	Button b5;//message
//	Button b6;//form
	...
	public void leftInit(){
		...
		new TableColumn(table,SWT.NONE).setText("产品描述");
	...
	public void rightInit(){
		...
		
		{
			...
			combo1.select(0);
			showProducts=new Button(cps,SWT.NONE);
			showProducts.setText("显示所有产品");
			showProducts.setLayoutData(createGridData(GridData.HORIZONTAL_ALIGN_FILL,2));
			showProducts.addSelectionListener(new SelectionAdapter(){
				public void widgetSelected(SelectionEvent e){
					String project=combo1.getText();
					if(project!=null){
						tv.setInput(ProcessManager.getProductsFromProject(project));
					}
				}
			});
		}
...
//		b6.setLayoutData(createGridData(GridData.FILL_HORIZONTAL,2));
		...
		query.addSelectionListener(new SelectionAdapter(){
			public void widgetSelected(SelectionEvent e){
				...
				List list=new ArrayList();
				if(project!=null){
					list=ProcessManager.getProductsFromProject(project);
					list=ProcessManager.queryProducts(list, b1.getSelection(),
							b2.getSelection(), b3.getSelection(), b4.getSelection(), 
							b5.getSelection(), combo2.getSelectionIndex(), text.getText());
					tv.setInput(list);
				}
			}
		});
