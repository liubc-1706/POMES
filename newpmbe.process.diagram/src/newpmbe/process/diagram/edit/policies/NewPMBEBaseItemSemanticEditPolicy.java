...
import newpmbe.process.diagram.edit.helpers.NewPMBEBaseEditHelper;
...
public class NewPMBEBaseItemSemanticEditPolicy extends SemanticEditPolicy {
	...
	protected Command getSemanticCommandSwitch(IEditCommandRequest req) {
		if (req instanceof CreateRelationshipRequest) {
			...
			//					|| !canConnectClockFlow((CreateRelationshipRequest) req))
			//				return null;
			//			//</lsz>
			//hzg
			if (!canConnect((CreateRelationshipRequest) req)) {
				//				MessageDialog.openError(null, "操作错误", "已有一条连接，不能再创建连接");
				return null;

			}
			if (!canConnectClockFlow((CreateRelationshipRequest) req))
				return null;
			//hzg
	 ...
	 * @generated NOT
	 */
	protected boolean canConnectClockFlow(CreateRelationshipRequest req) {
		if (NewPMBEElementTypes.EMPClockFlow_4001 == req.getElementType()) {
			EObject targetObject = req.getTarget();
			if (targetObject instanceof EMPActivity) {
				if (((EMPActivity) targetObject).getClockFlow() != null)
					return false;
			}
		}
		return true;
